...
class FRBRCluster(object):
    ...
    def get_main_creator(self, bib_object):
        ...
        if list_val_100abcd or list_val_110abcdn or list_val_111abcdn:
            if list_val_100abcd:
                self.main_creator.setdefault(prepare_name_for_indexing(list_val_100abcd[0]))
            if list_val_110abcdn:
                self.main_creator.setdefault(prepare_name_for_indexing(list_val_110abcdn[0]))
            if list_val_111abcdn:
                self.main_creator.setdefault(prepare_name_for_indexing(list_val_111abcdn[0]))
        ...
        # 3.1.1.2 - if there is no 1XX field, check for 7XX [CHECKED]
            ...
            self.main_creator.update(postprocess(prepare_name_for_indexing, list_val_700abcd))
            ...
            self.main_creator.update(postprocess(prepare_name_for_indexing, list_val_710abcdn))
            ...
            self.main_creator.update(postprocess(prepare_name_for_indexing, list_val_711abcdn))
    ...
    def create_manifestation(self):
        manifestation_to_add = FRBRManifestation()

        self.manifestation_from_original_raw_record = manifestation_to_add
    ...
    def merge_frbr_clusters_and_reindex(self,
        ...
        # perform these actions only if merging two existing frbr_clusters
        else:
    ...
    def match_and_index(self,
        ...
        if not matched_clusters:
            ...
            matched_cluster_to_merge_with = indexed_frbr_clusters_by_uuid.get(matched_clusters[0])

            self.merge_frbr_clusters_and_reindex(matched_cluster_to_merge_with,
                                                 indexed_frbr_clusters_by_uuid,
                                                 indexed_frbr_clusters_by_titles,
                                                 indexed_frbr_clusters_by_raw_record_id)
